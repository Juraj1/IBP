...
\abstract[cs]{V~této bakalářské práci se~autor zabývá návrhem a částečnou implementací radarového výškoměru. V této práci je kladen důraz na motulární architekturu a~proto je tento výškoměr navržen jako soubor samostatných modulů komunikujících pomocí BSD schránek. Implementace programového vybavení je v~C++ a~pro generování zvuku je použita knihovna PulseAudio. Dále je zde řešena bezpečnost mezivláknové fronty a~zásobníku pomocí třídy implementované jako šablona pro~zachování jednoduchosti a~obecnosti pro~další využití.} 
